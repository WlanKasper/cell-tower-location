    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn decode_wgs84(
        &self,
        param_wgs84_encoded: Option<models::Wgs84Encoded>,
        context: &C) -> Result<DecodeWgs84Response, ApiError>
    {
        ...
        // Body parameter
        let body = param_wgs84_encoded.map(|ref body| {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::Wgs84Decoded>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(DecodeWgs84Response::SuccessfulOperation
                    (body)
                )
            }
            400 => {
    ...
    async fn encode_wgs84(
        &self,
        param_wgs84_decoded: Option<models::Wgs84Decoded>,
        context: &C) -> Result<EncodeWgs84Response, ApiError>
    {
        ...
        let body = param_wgs84_decoded.map(|ref body| {
            serde_json::to_string(body).expect("impossible to fail to serialize")
        });
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::Wgs84Encoded>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(EncodeWgs84Response::SuccessfulOperation
                    (body)
                )
            }
            400 => {
